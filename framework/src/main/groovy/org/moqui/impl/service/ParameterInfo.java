...
public class ParameterInfo {
    ...
    public enum ParameterType { STRING, INTEGER, LONG, FLOAT, DOUBLE, BIG_DECIMAL, BIG_INTEGER, TIME, DATE, TIMESTAMP, LIST, SET, MAP, JSON_MAP }
    ...
    static {
        ...
        typeEnumByString.put("JsonMap", ParameterType.JSON_MAP);
    }
    ...
    public ParameterInfo(ServiceDefinition sd, MNode parameterNode) {
        ...
        if (parmType == ParameterType.JSON_MAP){
            parmClass = java.util.Map.class;
        }else{
                ...
                case JSON_MAP:
                    try {
                        ...
                        if (jsonVal instanceof Map) {
                            ...
                            converted = (Map) jsonVal;
                            break;
                        }

                        eci.messageFacade.addValidationError(null, namePrefix + name, serviceName,
                                "JsonMap must be a JSON object (map)", null);
                    } catch (Exception e) {
                        eci.messageFacade.addValidationError(null, namePrefix + name, serviceName,
                                "Could not convert JSON to JsonMap (Map)", e);
                    }
                    break;
            }
        }
